...
@Component({
  selector: 'app-pdfvivevk',
  templateUrl: './pdfvivevk.component.html',
  styleUrls: ['./pdfvivevk.component.css']
})
export class PdfvivevkComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //   const parentDoc :any = window.parent.window.document ;
    ...
    //   const compareDoc = async (doc1: { getPageIterator: (arg0: number) => any; } | any, doc2: { getPageIterator: (arg0: number) => any; } | any) => {
    ...
    //   const getPDFDocFromUpload = async (file: any, fileIndex: number) => {
    ...
    //   parentDoc.getElementById('fileUpload1').addEventListener('change', (e: { target: { files: any[]; }; }) => {
    //     getPDFDocFromUpload(e.target.files[0], 0);
    //   });
    
    //   parentDoc.getElementById('fileUpload2').addEventListener('change', (e: { target: { files: any[]; }; }) => {
